...
<template>
  <Scene class="bg-black max-w-[1980px] mx-auto">
      <div class="flex flex-col h-full justify-center items-center gap-12">
        <span id="logo" :class="['block rounded-full overflow-hidden h-[40vh] aspect-square bg-white p-4', { 'fade-out': fade }]">
          <img :src="logo"/>
        </span>
        <span v-if="countdown" id="countdown" :class="['text-white text-6xl font-semibold font-sourcesans uppercase text-center', { 'fade-out': fade }]">
